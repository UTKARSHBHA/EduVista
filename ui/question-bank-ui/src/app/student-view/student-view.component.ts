...
export class StudentViewComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private studentRegistrationService: StudentRegistrationService, private router: Router) {}

  ngOnInit(): void {
    const studentId = this.route.snapshot.paramMap.get('id');
    if (studentId) {
      this.studentRegistrationService.getStudentById(+studentId).subscribe(student => {
